...
public class VerkleWitness : IVerkleWitness
{
    ...
    public long AccessAccount(Address address, bool[] bitVector, bool isWrite = false)
    {
        ...
        for (int i = 0; i < bitVector.Length; i++)
        {
            if (bitVector[i])
            {
                gasUsed += AccessKey(AccountHeader.GetTreeKey(address.Bytes, UInt256.Zero, (byte)i), isWrite);
            }
        }
    ...
    {
        int[] reqSnapshot = _snapshots[snapshot];
        _accessedSubtrees.Restore(reqSnapshot[0]);
        _accessedLeaves.Restore(reqSnapshot[1]);
    }
