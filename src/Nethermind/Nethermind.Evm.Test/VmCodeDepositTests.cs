...
namespace Nethermind.Evm.Test
{
    ...
    public class VmCodeDepositTests : VirtualMachineTestsBase
    {
        ...
        public void Regression_mainnet_6108276()
        {
            ...
            Assert.AreEqual(new byte[] {0}, result.WithoutLeadingZeros().ToArray(), "storage reverted");
            Assert.AreEqual(98777, receipt.GasSpent, "no refund");

            byte[] returnData = Storage.Get(new StorageCell(TestItem.AddressC, 0));
            Assert.AreEqual(new byte[1], returnData.WithoutLeadingZeros().ToArray(), "address returned");
        }
        ...
        public void Regression_mainnet_226522()
        {
            ...
            byte[] returnData = Storage.Get(new StorageCell(TestItem.AddressC, 0));
            Assert.AreEqual(deployed.Bytes, returnData.WithoutLeadingZeros().ToArray(), "address returned");
        }
    }
}
