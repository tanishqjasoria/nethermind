...
{
    public class VerkleOneTimeChainProcessor : IBlockchainProcessor
    {
        public ITracerBag Tracers => _processor.Tracers;

        private readonly IBlockchainProcessor _processor;
        private readonly VerkleStateProvider _readOnlyStateProvider;

        private object _lock = new();

        public VerkleOneTimeChainProcessor(VerkleStateProvider stateProvider, IBlockchainProcessor processor)
        {
            _readOnlyStateProvider = stateProvider ?? throw new ArgumentNullException(nameof(stateProvider));
        ...
        public Block? Process(Block block, ProcessingOptions options, IBlockTracer tracer)
        {
            lock (_lock)
            {
                ...
                {
                    _readOnlyStateProvider.ClearTempChanges();
                }
